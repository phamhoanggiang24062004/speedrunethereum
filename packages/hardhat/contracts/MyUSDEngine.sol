...
contract MyUSDEngine is Ownable {
    ...
    function calculateCollateralValue(address user) public view returns (uint256) {
        ...
        return (s_userCollateral[user] * price) / PRECISION;
    }
    ...
    function _getCurrentExchangeRate() internal view returns (uint256) {
        ...
        uint256 interest = (totalDebt * borrowRate * timeElapsed)
    ...
    function setBorrowRate(uint256 newRate) external onlyRateController {
        if (newRate > 10_000) revert Engine__InvalidBorrowRate();
        _accrueInterest();
        borrowRate = newRate;
    ...
    function liquidate(address user) external {
        ...
        uint256 payout = collateralToCover + reward;
